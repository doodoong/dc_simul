...
//	static int slotThreshold = 0;
	...
	public static void main(String[] args) {
		...
		while(inputCheck)
		{
			System.out.println("Pvalue(<=1.0)  NumberOfStation(>=1)");
...
//					}
		...
		Simulator simulator = new Simulator(pvalue, numStation);
		simulator.run();
...
*/
			case 2 : //pvalue
				...
				for (int trial = 0; trial < maxTrial ; trial++) {
					...
					for (int i = 0; i < arraySize ; i += 1) {
						unit += 0.001;
						Simulator simulator_t = new Simulator(unit, numStation);
						simulator_t.run();
						meanDelayList[trial][i] = simulator_t.getMeanDelay();
						throughputList[trial][i] = simulator_t.getThroughput();
					}
			...
			case 3 : //numStation
				...
				for (int trial = 0; trial < maxTrial ; trial++) {
					for (int i = 1; i <= numStation; i += 1) {
						Simulator simulator_t = new Simulator(pvalue, i);
						simulator_t.run();
